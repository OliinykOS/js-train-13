...
function checkURLParameters(url, params) {
    ...
    for (const element of params) {
        if (urlObj.searchParams.has(element)) {
            Object.assign(result, { [element]: true });
        } else {
            Object.assign(result, { [element]: false });
        }
    }
    return result;
}
...
function manipulateQuery(url, options) {
    ...
    if (options.has(`append`)) {
        for (const element of options.get(`append`)) {
            urlObj.searchParams.append(element[0], element[1]);
        }
    }
...
function sortUrlParams(url) {
    ...
    // Повертаємо новий URL як рядок.
    let urlObj = new URL(url);
    let params = Array.from(urlObj.searchParams.entries()).sort();
    for (const key of params) {
        urlObj.searchParams.delete(key[0]);
    }
    for (const key of params) {
        urlObj.searchParams.append(key[0], key[1]);
    }
    return urlObj.toString();
}
